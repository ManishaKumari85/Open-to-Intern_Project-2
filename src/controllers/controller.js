...
const {validateEmail,isValid} = require("../validator/validator");
...
const createCollege = async function(req,res){
   ...
   const createIntern = async function (req, res) {
    try {
      ...
      if (!(data.email)){return res.status(400).send({ status: false, msg: "Intern email is required" })}
      if (!isValid(data.collegeName)) {return res.status(400).send({ status: false, msg: "College name is required is required" })}

      if (!validateEmail(data.email)) return res.status(400).send({ status: false, msg: "please enter a valid email" })
